...
export const AdminDashboard: React.FC = () => {
  ...
  // Sincronizar DateRangePicker quando o período mudar (exceto 'custom')
  useEffect(() => {
    if (dateFilter !== 'custom') {
      const { start, end } = getDateRange();
      setCustomStartDate(formatDateForInput(start));
      // Para períodos não-custom, definir data final apenas se for diferente da inicial
  ...
  const getDateRange = (): { start: number; end: number } => {
    ...
    switch (dateFilter) {
      ...
      case 'semana': {
        // FILTRO "SEMANA": calcula semana de SEGUNDA a SÁBADO usando timezone local
        // getDay() retorna: 0=Domingo, 1=Segunda, 2=Terça, 3=Quarta, 4=Quinta, 5=Sexta, 6=Sábado
        const dayOfWeek = today.getDay();
        
        // Calcular quantos dias subtrair para chegar à segunda-feira da semana atual
        // Se hoje é domingo (0), subtrai 6 dias para chegar à segunda anterior
        // Se hoje é segunda (1), não subtrai nada
        // Se hoje é terça (2), subtrai 1 dia
        // Se hoje é sábado (6), subtrai 5 dias
        ...
        // Fim da semana = sábado (23:59:59.999) - 5 dias após a segunda-feira
      ...
      case 'semanaPassada': {
        // FILTRO "SEMANA PASSADA": calcula semana passada de SEGUNDA a SÁBADO usando timezone local
        const dayOfWeek = today.getDay();
        ...
        // Fim da semana passada = sábado (23:59:59.999) - 5 dias após a segunda-feira da semana passada
  ...
  // Função para formatar período de forma minimalista
    ...
    switch (dateFilter) {
      ...
      case 'custom':
        ...
        if (customStartDate && customEndDate) {
          ...
          const start = new Date(y1, m1 - 1, d1).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
          const end = new Date(y2, m2 - 1, d2).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' });
          return `${start} - ${end}`;
        }
        ...
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 mb-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <DateRangePicker
                startDate={customStartDate || formatDateForInput(start)}
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              {customStartDate && customEndDate && customStartDate === customEndDate
                ? `Exibindo dados do dia ${(() => { const [y, m, d] = customStartDate.split('-').map(Number); return new Date(y, m - 1, d).toLocaleDateString('pt-BR'); })()}`
                : customStartDate && customEndDate
                ? `Período: ${(() => { const [y, m, d] = customStartDate.split('-').map(Number); return new Date(y, m - 1, d).toLocaleDateString('pt-BR'); })()} até ${(() => { const [y, m, d] = customEndDate.split('-').map(Number); return new Date(y, m - 1, d).toLocaleDateString('pt-BR'); })()} (${workingDays} dia${workingDays !== 1 ? 's' : ''} útil${workingDays !== 1 ? 'is' : ''})`

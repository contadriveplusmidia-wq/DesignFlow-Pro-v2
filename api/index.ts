...
app.post('/api/demands', async (req: Request, res: Response) => {
  try {
    const { userId, userName, items, totalQuantity, totalPoints, timestamp: providedTimestamp } = req.body;
    const id = `demand-${Date.now()}`;
    // Usar timestamp fornecido pelo frontend ou Date.now() como fallback
    // Verificar explicitamente se foi fornecido um timestamp válido (número maior que 0)
    const timestamp = typeof providedTimestamp === 'number' && providedTimestamp > 0 
      ? providedTimestamp 
      : Date.now();
